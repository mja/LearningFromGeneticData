...
$$\mathrm{var}(P) = \mathrm{var}(𝐺) + \mathrm{var}(𝐸)$$
**Proportion of variance**
$$h^2 = \frac{\mathrm{var}(𝐺)}{\mathrm{var}(𝑃)}$$
...
then the parent–offspring covariance simplifies to
...
n_sim <- 500
parents <- rnorm(n_sim, mean = 0, sd = 1)
children_h201 <- 0.1 * parents + rnorm(n_sim, mean=0, sqrt(1 - 0.1))
children_h209 <- 0.9 * parents + rnorm(n_sim, mean=0, sqrt(1 - 0.9))

sim01 <- data.frame(parents = parents, children = children_h201, h2 = "h^2 = 0.1")
sim09 <- data.frame(parents = parents, children = children_h209, h2 = "h^2 = 0.9")

ggplot(bind_rows(sim01, sim09), aes(x = parents, y = children)) +
  geom_point() +
  stat_smooth(method = lm, formula = y ~ x, se = FALSE) +
